...
kubectl apply -f manifests/01-rbac-role.yaml     	# Role: read-only pods in default ns
kubectl apply -f manifests/02-rbac-rolebinding.yaml  # Bind to ServiceAccount lab-viewer
kubectl get role,rolebinding -n default	# Confirm they exist

# Test as the ServiceAccount

kubectl apply -f manifests/03-test-pod-as-sa.yaml	# Pod running as lab-viewer SA
...
```bash
# Deploy a secret
kubectl apply -f manifests/04-opaque-secret.yaml           # Create a secret
...
kubectl apply -f manifests/05-secret-consumer-pod.yaml     # Pod consuming via both methods
...
kubectl apply -f manifests/08-server-pod.yaml  	# nginx server, label: role=server
kubectl apply -f manifests/09-client-pod.yaml  	# curl client, label: role=client
...
kubectl apply -f manifests/06-netpol-deny-all-ingress.yaml  # Block ALL ingress to server
...
kubectl apply -f manifests/07-netpol-allow-labeled.yaml  # Allow only pods with role=client
kubectl exec client-pod -- curl -s http://$(kubectl get pod server-pod -o jsonpath='{.status.podIP}'):80
...
kubectl apply -f manifests/13-non-compliant-pod.yaml
# Expected: Error - violates PodSecurity restricted

# Compliant Pod (should be ACCEPTED)
kubectl apply -f manifests/12-compliant-pod.yaml
