...
4. Inspect the helloworld container with ctr `sudo ctr  --namespace moby container info <container id>`
5. Look at running containers with runc `sudo runc --root /run/docker/runtime-runc/moby list`
6. inspect the helloworld container with runc `sudo runc --root /run/docker/runtime-runc/moby state <container id>`

## LAB 3 - The “hello world” Container - Digging Deeperer
1. Inspect the helloworld container with runc `sudo runc --root /run/docker/runtime-runc/moby state <container id>` and note the pid
...
5. Look at the root file system of helloworld with nsenter `sudo nsenter -t <pid> -–root ls -la /`
6. Open a shell in the container using nsenter `sudo nsenter -t <pid> --root --mount --pid --ipc --uts --net --time --cgroup  -- /bin/bash -l`
7. List the file structure `ls -la`
8. Type `exit`
9  Open a shell with docker's cli `docker exec -it helloworld /bin/bash`
...
6. Lets untar one of the image layers. `mkdir -p nginx/blob_contents && tar -xf nginx/blobs/sha256/669e0ab8e7fa25a26343d0091ccd0b2548326f44f723ef349841c04fd99b76ef -C nginx/blob_contents`
